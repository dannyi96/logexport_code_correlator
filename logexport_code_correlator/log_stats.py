...
def get_splunk_job_stats(sid):
    try:
        reload_cookies()
        job_details_request = requests.get(BASE_URL + "/%s?output_mode=json"%(sid), headers=HEADERS)
...
def get_splunk_event_bytes(sid):
    reload_cookies()
    get_events_request = requests.get(BASE_URL + "/%s/results_preview?output_mode=json_rows&count=1&offset=0&show_metadata=true&add_summary_to_metadata=false&_=1645542148018"%(sid), headers=HEADERS)
...
def stop_splunk_job(sid):
    reload_cookies()
    stop_job_request = requests.post(BASE_URL + "/%s/control"%(sid), headers=HEADERS, data='output_mode=json&action=finalize')
...
def get_stats(sid):
    total_time=0
    while(True):
        job_stats = get_splunk_job_stats(sid)
        if(total_time>=120):
            break
    ...
    total_bytes = 0 if job_stats[1] == 0 else get_splunk_event_bytes(sid)
...
def prepare_batch_query(log_queries):
    return " OR ".join([ "( " + log_query[1] + " )" for log_query in log_queries])

def dump_batch_to_csv(batched_rows, tot_events, tot_bytes):
    new_rows = []
    new_tot_events = "<"+str(tot_events) if strip_metachars(tot_events) != 0 else tot_events
    new_tot_bytes = "<"+str(tot_bytes) if strip_metachars(tot_bytes) != 0 else tot_bytes
    for row in batched_rows:
        new_rows.append([row[0], row[1], new_tot_events, new_tot_bytes])
    
    dump_csv(new_rows)

def strip_metachars(stat):
    return int(str(stat).replace(">","").replace("<",""))

def error_handle_stats(events, bytes):
    events = strip_metachars(events)
    bytes = strip_metachars(bytes)
    if events in (-1, -2) or bytes in (-1, -2):
        print('Exiting as tot_events=%s, tot_bytes=%s'%(events, bytes))
        exit()

def batch_analyse_dump(log_queries, threshold):
    batched_query = prepare_batch_query(log_queries)
    ...
    sid = createSplunkJob(query)
    if sid == "UNAUTHORIZED" or sid == 'Job Creation Failed':
        print('Exiting for sid=%s'%(sid))
        exit()
    time.sleep(10)
    tot_events, tot_bytes = get_stats(sid)
    error_handle_stats(tot_events, tot_bytes)
    dump_status = False
    if strip_metachars(tot_events) <= threshold:
        dump_batch_to_csv(log_queries, tot_events, tot_bytes)
        dump_status = True
    return dump_status

def sequence_analyse_dump(log_queries):
    for log_query in log_queries:
        ...
        tot_events, tot_bytes = get_stats(sid)
        error_handle_stats(tot_events, tot_bytes)
        dump_csv([[ log_query[0], log_query[1], tot_events, tot_bytes]])

def analyse_log_csv(file_name, start_index, batch_size, threshold):
    with open(file_name) as csv_file:
        ...
        for row_no in range(start_index, len(lines), batch_size):
            ...
            batch_analyse_dump_status = batch_analyse_dump(batched_rows, threshold)
            if batch_analyse_dump_status == False:
                sequence_analyse_dump(batched_rows)
